...
void CTable::UpdateSetWhere(string order)
{
	...
	size_t desEqual = order.find("=", WHEREpos + 1); size_t desMore = order.find(">", WHEREpos + 1); size_t desLess = order.find("<", WHEREpos + 1);
	
	int desRow[MAX_DATA] = { 0 }, i_desRow = 0;
	for (int i = 0; i < MAX_DATA; ++i)
		desRow[i] = -1;

	if (desEqual != string::npos) {
	...
	else if (desMore != string::npos) {
		int numData = 0;
		for (numData = 0; isArrOneDimen(&pTa->_pTable->_data[numData][0], MAX_DATA) != true; ++numData);  // 计算data行数
		string desPro = order.substr(WHEREpos + 6, desMore - 1 - WHEREpos - 6), desData = order.substr(desMore + 2, order.length() - desMore - 2); 
		for (int i = 0; !pTa->_pTable->_property[i].empty(); ++i)
		{
			if (pTa->_pTable->_property[i] == desPro)
			{
	...
	else if (desLess != string::npos) {
		int numData = 0;
		for (numData = 0; isArrOneDimen(&pTa->_pTable->_data[numData][0], MAX_DATA) != true; ++numData);  // 计算data行数
		string desPro = order.substr(WHEREpos + 6, desLess - 1 - WHEREpos - 6), desData = order.substr(desLess + 2, order.length() - desLess - 2); 
		for (int i = 0; !pTa->_pTable->_property[i].empty(); ++i)
		{
			if (pTa->_pTable->_property[i] == desPro)
			{
...
// SELECT * FROM student ORDER BY Score2, Score3 ASC WHERE Score1 > 90（大于小于等于三种情况）
